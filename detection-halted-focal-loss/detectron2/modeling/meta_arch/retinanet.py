...
from .focal_loss import sigmoid_focal_loss_jit
from .encourage_loss import sigmoid_mixed_loss_jit, sigmoid_encourage_loss_jit
...
class RetinaNet(nn.Module):
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        ...
        # loss_clsz, base_loss, add_loss, power, beta
        # if 'alpha'

        if self.cfg.loss_clsz == 'ml':
            loss_cls = sigmoid_mixed_loss_jit(
        ...
        elif self.cfg.loss_clsz == 'el':
            loss_cls = sigmoid_encourage_loss_jit(
                cat(pred_logits, dim=1)[valid_mask],
                gt_labels_target.to(pred_logits[0].dtype),
                alpha=self.focal_loss_alpha,
                gamma=self.focal_loss_gamma,
                ...
                add_loss=self.cfg.add_loss,
                log_end= self.cfg.log_end,
                power=self.cfg.power,
                beta=self.cfg.beta,
                ita=self.cfg.ita,
                el_start=self.cfg.el_start,
        ...
        if self.num_steps % 10000 == 0:
            with open('log_steps.txt', 'a+') as f:
                f.write('cur step:'+ str(self.num_steps) +'\n')
        # loss_cls = sigmoid_focal_loss_jit(
        #     cat(pred_logits, dim=1)[valid_mask],
        #     gt_labels_target.to(pred_logits[0].dtype),
        #     alpha=self.focal_loss_alpha,
