            ...
            if bg != 0:
                if self.opt.base_loss in ['mse','mae','mse_sigmoid','mae_sigmoid']:
                    ...
                    if bg > 0:  # power bonus
                        ...
                        if self.opt.sub_data_rho < 1:
                            bonus = torch.log(
                                torch.clamp((torch.ones_like(probs) - probs*self.opt.sub_data_rho), min=self.cl_eps))  # sub less prob on the data
                        else:
                            ...
                            if self.opt.log_end != 1.0:  # e.g. 0.9
                                ...
                                if self.opt.el_schedule_le == 1:  # qadd in 2021/4/20 night
                                    rho = 1
                    ...
                    if bonus is not None:
                        ...
                        rho = 1
                        if self.opt.el_schedule is not None and self.opt.el_schedule_le == 0:
                            cur_time = float(self.get_epoch()) / self.opt.el_schedule_max_epoch  # t/T
                            if self.opt.el_schedule == 'linear':
                                rho = cur_time
                            elif self.opt.el_schedule == 'exp':
                        ...
                        if self.opt.label_smoothing != 0:
                            ...
                            c_loss = c_loss * (
                                        1 - self.opt.label_smoothing) + self.opt.label_smoothing * smoothing_c_loss
                    # c_loss = F.nll_loss(
