...
class EfficientNetLS(nn.Module):
    ...
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, num_classes=1000, gpuid=0, margin=1):
        ...
        self.out_fc= nn.Linear(last_channels, 256)
        self.classifier = LSoftmaxLinear(input_features=256, output_features=num_classes, margin=margin, device=self.device)
    ...
    def forward(self, x,target=None):
        x = self.features(x)
        x = x.mean([2, 3])
        x = self.classifier(self.out_fc(F.dropout(x, self.dropout_rate)), target=target)
        return x

def _efficientnetls(arch, pretrained, progress, gpuid=0, margin=1,**kwargs):
    width_mult, depth_mult, _, dropout_rate = params[arch]
    model = EfficientNetLS(width_mult, depth_mult, dropout_rate,gpuid=gpuid, margin=margin, **kwargs)
    if pretrained:
...
@mlconfig.register
def efficientnetls_b0(pretrained=False, progress=True, gpuid=0, margin=1, **kwargs):
    return _efficientnetls('efficientnet_b0', pretrained, progress,gpuid=gpuid, margin=margin, **kwargs)